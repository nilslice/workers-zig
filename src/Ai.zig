...
pub fn textEmbeddings(self: *const Ai, model: []const u8, input: TextEmbeddingsInput) !TextEmbeddingsOutput {
    ...
    for (input.text, 0..) |t, i| {
        ...
        try writeJsonString(&w.writer, t);
    }
...
pub fn translation(self: *const Ai, model: []const u8, input: TranslationInput) !TranslationOutput {
    ...
    try writeKey(&w.writer, "text", &need_comma);
    try writeJsonString(&w.writer, input.text);
    try writeKey(&w.writer, "target_lang", &need_comma);
    try writeJsonString(&w.writer, input.target_lang);
    if (input.source_lang) |v| {
        try writeKey(&w.writer, "source_lang", &need_comma);
        try writeJsonString(&w.writer, v);
    }
...
pub fn summarization(self: *const Ai, model: []const u8, input: SummarizationInput) !SummarizationOutput {
    ...
    try writeKey(&w.writer, "input_text", &need_comma);
    try writeJsonString(&w.writer, input.input_text);
    if (input.max_length) |v| {
        try writeKey(&w.writer, "max_length", &need_comma);
...
pub fn imageToText(self: *const Ai, model: []const u8, input: ImageToTextInput) !ImageToTextOutput {
    ...
    if (input.prompt) |prompt| {
        try writeKey(&w.writer, "prompt", &need_comma);
        try writeJsonString(&w.writer, prompt);
    }
    if (input.max_tokens) |v| {
        try writeKey(&w.writer, "max_tokens", &need_comma);
        try w.writer.print("{d}", .{v});
    }
...
pub fn textToImage(self: *const Ai, model: []const u8, input: TextToImageInput) ![]const u8 {
    ...
    if (is_multipart) {
        try writeKey(&w.writer, "__multipart", &need_comma);
        try w.writer.writeAll("true");
    }
    try writeKey(&w.writer, "prompt", &need_comma);
    try writeJsonString(&w.writer, input.prompt);
    if (input.negative_prompt) |v| {
        try writeKey(&w.writer, "negative_prompt", &need_comma);
        try writeJsonString(&w.writer, v);
    }
    if (input.height) |v| {
        try writeKey(&w.writer, "height", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.width) |v| {
        try writeKey(&w.writer, "width", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.num_steps) |v| {
        // Flux models use "steps" instead of "num_steps"
        const key = if (is_multipart) "steps" else "num_steps";
        try writeKey(&w.writer, key, &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.guidance) |v| {
        try writeKey(&w.writer, "guidance", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.seed) |v| {
        try writeKey(&w.writer, "seed", &need_comma);
        try w.writer.print("{d}", .{v});
    }
...
pub fn textToSpeech(self: *const Ai, model: []const u8, input: TextToSpeechInput) ![]const u8 {
    ...
    try writeJsonString(&w.writer, input.prompt);
    if (input.lang) |v| {
        try writeKey(&w.writer, "lang", &need_comma);
        try writeJsonString(&w.writer, v);
    }
...
pub fn speechToTextWebSocket(self: *const Ai, model: []const u8, input: SpeechToTextWsInput) !Response {
    ...
    try writeKey(&w.writer, "encoding", &need_comma);
    try writeJsonString(&w.writer, input.encoding);
    try writeKey(&w.writer, "sample_rate", &need_comma);
    try writeJsonString(&w.writer, input.sample_rate);
    if (input.interim_results) |v| {
        try writeKey(&w.writer, "interim_results", &need_comma);
        try w.writer.writeAll(if (v) "true" else "false");
    }
    if (input.language) |v| {
        try writeKey(&w.writer, "language", &need_comma);
        try writeJsonString(&w.writer, v);
    }
...
fn buildTextGenJson(allocator: std.mem.Allocator, input: TextGenerationInput, options: ?AiOptions) ![]const u8 {
    ...
    if (input.messages) |messages| {
        try writeKey(&w.writer, "messages", &need_comma);
        try writeMessagesJson(&w.writer, messages);
    }
    if (input.max_tokens) |v| {
        try writeKey(&w.writer, "max_tokens", &need_comma);
        try w.writer.print("{d}", .{v});
    ...
    if (input.temperature) |v| {
        try writeKey(&w.writer, "temperature", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.top_p) |v| {
        try writeKey(&w.writer, "top_p", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.top_k) |v| {
        try writeKey(&w.writer, "top_k", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.seed) |v| {
        try writeKey(&w.writer, "seed", &need_comma);
    ...
    if (input.repetition_penalty) |v| {
        try writeKey(&w.writer, "repetition_penalty", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.frequency_penalty) |v| {
        try writeKey(&w.writer, "frequency_penalty", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.presence_penalty) |v| {
        try writeKey(&w.writer, "presence_penalty", &need_comma);
        try w.writer.print("{d}", .{v});
    }
    if (input.lora) |v| {
        try writeKey(&w.writer, "lora", &need_comma);
        try writeJsonString(&w.writer, v);
    }
    if (input.tools) |tools| {
        try writeKey(&w.writer, "tools", &need_comma);
        try writeToolsJson(&w.writer, tools);
    }
    if (input.response_format) |rf| {
        try writeKey(&w.writer, "response_format", &need_comma);
        try w.writer.writeAll("{\"type\":");
        try writeJsonString(&w.writer, rf.type);
        if (rf.json_schema) |schema| {
            try w.writer.writeAll(",\"json_schema\":");
            try w.writer.writeAll(schema); // raw JSON pass-through
        }
        try w.writer.writeAll(&.{'}'});
    }
    if (options) |opts| {
        try writeKey(&w.writer, "__options", &need_comma);
        try writeOptionsJson(&w.writer, opts);
    }
...
fn writeMessagesJson(writer: *std.Io.Writer, messages: []const Message) !void {
    try writer.writeAll(&.{'['});
        ...
        if (msg.content_parts) |parts| {
            ...
            for (parts, 0..) |part, j| {
                ...
                if (part.image_url) |img| {
                    ...
                    try writeJsonString(writer, img.url);
                    try writer.writeAll(&.{'}'});
                }
                try writer.writeAll(&.{'}'});
            }
...
fn writeToolsJson(writer: *std.Io.Writer, tools: []const ToolDefinition) !void {
    try writer.writeAll(&.{'['});
    for (tools, 0..) |tool, i| {
        if (i > 0) try writer.writeAll(&.{','});
        try writer.writeAll("{\"type\":");
        try writeJsonString(writer, tool.type);
        try writer.writeAll(",\"function\":{\"name\":");
        try writeJsonString(writer, tool.function.name);
        try writer.writeAll(",\"description\":");
        try writeJsonString(writer, tool.function.description);
        if (tool.function.parameters) |params| {
            try writer.writeAll(",\"parameters\":");
            try writer.writeAll(params); // raw JSON pass-through
        }
        try writer.writeAll("}}");
    }
    try writer.writeAll(&.{']'});
...
fn writeOptionsJson(writer: *std.Io.Writer, opts: AiOptions) !void {
    try writer.writeAll(&.{'{'});
    var need_comma = false;
    if (opts.gateway) |gw| {
        try writeKey(writer, "gateway", &need_comma);
        try writer.writeAll("{\"id\":");
        try writeJsonString(writer, gw.id);
        if (gw.cache_key) |v| {
            try writer.writeAll(",\"cacheKey\":");
            try writeJsonString(writer, v);
        }
        if (gw.cache_ttl) |v| {
            try writer.writeAll(",\"cacheTtl\":");
            try writer.print("{d}", .{v});
        }
        if (gw.skip_cache) |v| {
            try writer.writeAll(",\"skipCache\":");
            try writer.writeAll(if (v) "true" else "false");
        }
        try writer.writeAll(&.{'}'});
    }
    if (opts.tags) |tags| {
        try writeKey(writer, "tags", &need_comma);
        try writer.writeAll(&.{'['});
        for (tags, 0..) |tag, i| {
            if (i > 0) try writer.writeAll(&.{','});
...
fn writeKey(writer: *std.Io.Writer, key: []const u8, need_comma: *bool) !void {
...
test "writeJsonString — plain text" {
    ...
    try writeJsonString(&w.writer, "hello");
...
test "writeJsonString — escapes special characters" {
    var w = std.Io.Writer.Allocating.init(testing.allocator);
    try writeJsonString(&w.writer, "a\"b\\c\nd\te");
    const result = try w.toOwnedSlice();
    defer testing.allocator.free(result);
...
test "writeJsonString — escapes control characters" {
    var w = std.Io.Writer.Allocating.init(testing.allocator);
    try writeJsonString(&w.writer, &[_]u8{ 0x01, 0x1F });
    const result = try w.toOwnedSlice();
    defer testing.allocator.free(result);
