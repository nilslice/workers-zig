...
pub const Step = struct {
    ...
    pub fn do(self: Step, name: []const u8, config: StepConfig, comptime callback: anytype) ![]const u8 {
        ...
        var config_json: std.ArrayListUnmanaged(u8) = .empty;
        try buildStepConfigJson(self.allocator, config, &config_json);
        const config_str = config_json.items;
...
fn buildStepConfigJson(allocator: std.mem.Allocator, config: StepConfig, out: *std.ArrayListUnmanaged(u8)) !void {
    const w = out.writer(allocator);
    try w.writeByte('{');
    var has_field = false;

    if (config.retries) |r| {
        try w.writeAll("\"retries\":{\"limit\":");
        try std.fmt.format(w, "{d}", .{r.limit});
        try w.writeAll(",\"delay\":\"");
        try w.writeAll(r.delay);
        try w.writeAll("\",\"backoff\":\"");
        try w.writeAll(r.backoff.toString());
        try w.writeAll("\"}");
        has_field = true;
    }
    if (config.timeout) |t| {
        if (has_field) try w.writeByte(',');
...
test "StepConfig JSON — empty" {
    ...
    try buildStepConfigJson(std.testing.allocator, .{}, &out);
...
test "StepConfig JSON — retries only" {
    var out: std.ArrayListUnmanaged(u8) = .empty;
    try buildStepConfigJson(std.testing.allocator, .{
