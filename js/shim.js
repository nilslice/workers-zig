...
const wasi_imports = {
  ...
  // Stubs for anything else the linker might pull in.
...
export default {
  ...
  async connect(socket, workerEnv, ctx) {
    await init();
    if (!_inst.exports.connect) return;

    const sh = store(socket);
    const envH = store(workerEnv);
    const ch = store(ctx);

    try {
      const connectFn = HAS_JSPI
        ? WebAssembly.promising(_inst.exports.connect)
        : _inst.exports.connect;

      HAS_JSPI
        ? await connectFn(sh, envH, ch)
        : connectFn(sh, envH, ch);
    } catch (e) {
      console.error("[workers-zig] connect handler error:", e);
      throw e;
    } finally {
      drop(sh);
      drop(envH);
